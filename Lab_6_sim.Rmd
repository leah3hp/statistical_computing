...
```{r}
st_norm = rnorm(1000,mean = 0, sd = 1)
head(st_norm,5)

beta = rbeta(20,0.1,0.1)
head(beta,5)

char_samp = sample(c("A","G","C","T"),2000,replace = TRUE,prob = c(0.25,0.25,0.25,0.25))
head(char_samp,5)

x = runif(100,0,1)
y = runif(100,0,x[1:100])
data_unif = as.data.frame(cbind(x,y))
head(data_unif,5)
...
#N(-3,10)
rfun_norm = function(n.max) {return(rnorm(n.max,mean = -3,sd = sqrt(10)))}

#Exp(5)
rfun_exp = function(n.max) {return(rexp(n.max,rate = .2))}

#Beta(1,1)
rfun_beta = function(n.max) {return(rbeta(n.max,1,1))}

par(mfrow=c(1,3))
plot.cum.means(rfun_norm,n.max)
plot.cum.means(rfun_exp,n.max)
plot.cum.means(rfun_beta,n.max)

#Yes, as the sample size increases the sample means approach their expected values based on their probability density functions.
```
...
```{r}
pfun_norm = function(d){return(pnorm(d,mean = -3,sd = sqrt(10)))}
pfun_exp = function(q){return(pexp(q,0.2))}
pfun_beta = function(q){return(qbeta(q,1,1))}

par(mfrow=c(1,3))
plot.ecdf(rfun = rfun_norm,pfun = pfun_norm,c(10,100,1000))
plot.ecdf(rfun = rfun_exp,pfun = pfun_exp,c(10,100,1000))
plot.ecdf(rfun = rfun_beta,pfun = pfun_beta,c(10,100,1000))
```
...
simulate.data = function(n = 60,mu.drug = 2){
  ...
  nodrug = 100*rexp(n, rate=1/mu.nodrug)
  return_list = list(nodrug,drug)
  names(return_list) = c("nodrug","drug")
  return(return_list)
}
```
...
head(results1$nodrug,6)
head(results1$drug,6)

results2 = simulate.data()
head(results2$nodrug,6)
head(results2$drug,6)
```
...
```{r}
dif_no_drug = abs(mean(results1$nodrug)-mean(results2$nodrug))
dif_drug = abs(mean(results1$drug)-mean(results2$drug))
dif_results1 = abs(mean(results1$nodrug) - mean(results1$drug))

dif_no_drug
dif_drug
dif_results1

#The biggest absolute difference is between the drug group and the non-drug group within the first results sample. This makes the most sense assuming that there is a drug effect, because the other two are comparing differences in sample means between samples from the same simulation so they should not deviate much assuming a relativley low sample variance. 
```
...
plot.data = function(data){
    #get the two groups for plotting from the data
    x.drug = data$drug
    x.nodrug = data$nodrug
...
#There are differences in the shape of smoothness in the distribution. The scales are a bit different and we see more of an exponentially smopth decay in this plot.
```
...
simulate.difference = function(n = 60, mu.drug = 2){ 
  temp_list = simulate.data(n,mu.drug)
  return(mean(temp_list$drug)-mean(temp_list$nodrug))
}
simulate.difference()
simulate.difference()
...
rep.sim = function(nreps = 200,n = 60,mu.drug = 2,seed = NULL){
  temp_int = 0
  for(i in 1:nreps){
    temp_return = simulate.difference(n,mu.drug)
    if(temp_return >= 100){
      temp_int = temp_int + 1
    }
  }
...
```{r}
n = rep(5:100)
test_vec = rep(0,length(n))
  for(j in 1:length(test_vec)){
    test_vec[j] = rep.sim(n = n[j]) #initial vector of 0's then with each it we will impute new values from rep sim using a specific n from 5-100
}

plot(n,test_vec,xlab = "number of patients",ylab = "# of trials (out of 200) w/ mean difference > 100%")

smallest_n = min(n[test_vec >=190])
smallest_n

#the smalledst size is n=33.
```
...
```{r}
mu_drug_test = seq(0,5,0.25) #drug mean effects 
test_vec_2 = rep(0,length(mu_drug_test))

for(k in 1:length(test_vec_2)){
  test_vec_2[k] = rep.sim(n = 20, mu.drug = mu_drug_test[k]) 
}

plot(mu_drug_test,test_vec_2,xlab = "mean drug effect",ylab = "# of trials (out of 200) w/ mean difference > 100%")

smallest_n_2 = min(mu_drug_test[test_vec_2 >=190])
smallest_n_2

#mu_dru must be at lease 2.25 to exceed 190.
```
...
#New rep.sim function
rep.sim.new = function(nreps = 200,n,mu.drug,seed = NULL){
  temp.drug.mat = matrix(rep(0,1000),nrow = 200,ncol = 5)
  temp.nodrug.mat = matrix(rep(0,1000),nrow = 200,ncol =5)
  resp = list(temp.drug.mat,temp.nodrug.mat)
  for(i in 1:nreps){
    temp.resp = simulate.data(n,mu.drug)
    resp[[1]][i,] = temp.resp$drug
    resp[[2]][i,] = temp.resp$nodrug
  }
  return(resp)
}
...
for(i in 1:12){
  ...
  if(i == 1){
    patients = rep.sim.new(n = 5,mu.drug = 1.5)
  }else{
    patients_new = rep.sim.new(n=5,mu.drug = 1.5)
    drug_old = patients[[1]]
    drug_new = patients_new[[1]]
    nodrug_old = patients[[2]]
    nodrug_new = patients_new[[2]]
    drug = cbind(drug_old,drug_new)
    nodrug = cbind(nodrug_old,nodrug_new)
    patients = list(drug,nodrug)
  }

  #Test if we can stop
  temp_count = rep(0,200)
  for(j in 1:200){
      test_val = mean(patients[[1]][j,]) - mean(patients[[2]][j,])
      if(test_val >=100){
        temp_count[j] = 1
      }
  }
  if(sum(temp_count) >= 190){
    #SUCCESS
    response = c(5*i,sum(temp_count))
    break
  }else if(i == 12){
    #FAILURE
    response = "FAILURE"
...
#this function will either give falure or a number above 100 (most of the time). Compared to 3a I am getting less than 130 and this makes sense as this is the more appropriate way to do this test sampling.
```
...
ab.collect= function(n, a.mean, a.sd, b.sd, b.mean){
  a.response=rnorm(n=n, mean=a.mean, sd=a.sd)
  b.response=rnorm(n=n, mean=b.mean, sd=b.sd)
  new.list=list(a.response, b.response)
  names(new.list)=c("a.response", "b.response")
  return(new.list)
}

temp=ab.collect(10,0,1,0,1)
temp
mean(temp$a.response)


temp_2=ab.collect(5,0,1,0,1)
temp_2
mean(temp_2$b.response)
...
test.at.end=function(n, a.mean, a.sd, b.mean, b.sd){
  test_samp=ab.collect(n, a.mean, a.sd, b.sd, b.mean)
  test=t.test(test_samp$a.response,test_samp$b.response)
  p_val=test$p.value
  if (p_val<=0.5){
    if(mean(test_samp$a.response)<= mean(test_samp$b.response)){
      return("B")
    }else{
      return("A")
    }
...
test_samp2=test.at.end(n=2000,a.mean=100,a.sd=20, b.mean=140, b.sd=10)
test_samp2
#command-shift-c
```
...
test.as.you.go=function(n.per.day, n.days, a.mean, a.sd, b.mean, b.sd){
  for(i in 1:n.days){
    if (i==1){
      data_act=ab.collect(n.per.day, a.mean, a.sd, b.mean, b.sd)
    }else{
      data_new=ab.collect(n.per.day, a.mean, a.sd, b.mean, b.sd)
      a.group_temp =c(data_act$a.response, data_new$a.response)
      b.group_temp=c(data_act$b.response, data_new$b.response)
      data_act= list(a.group_temp,b.group_temp)
      names(data_act)=c("a.response", "b.response")
    }
    daily_test=t.test(data_act$a.response,data_act$b.response )
    daily_pval=daily_test$p.value
    if (daily_pval<=0.5){
       if(mean(data_act$a.response)<= mean(data_act$b.response)){
         resp=list(data_act$b.response)
         names(resp)="b.response"
      return(resp)
    }else{
      resp=list(data_act$a.response)
         names(resp)="a.response"
      return(resp)
      break
    }
  }else if (i==n.days){
...
test_proc_2=test.as.you.go(n.per.day=100, n.days=20, a.mean=100, a.sd=20, b.mean=140, b.sd=10)
test_proc_2
class(test_proc_2)

#here you do save time since it will take fewer days but will get the same results
...
test.at.end.result = test.at.end(n = 1000,a.mean = 100,a.sd = 20,b.mean = 100,b.sd = 20)
test.as.you.go.result = test.as.you.go(n.per.day = 100,n.days = 10,a.mean = 100,a.sd = 20,b.mean =100,b.sd = 20)

test.at.end.result
test.as.you.go.result

#. These resluts are different, so it can be a bad approach. This is because you coud get significant results on day 1 and stop but then if you do it all at once you will get different results. The second test should have more As and Bs and so on like this until 60. 
```
